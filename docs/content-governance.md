...
- A long, fact-dense chapter will have more questions, because it supports more non-overlapping learning objectives.
- Do not pad a chapter with near-duplicate or trivial questions just to hit a round number.
- Do not omit a well-supported, distinct fact just to keep chapters "even."
...
- The question stem is direct and self-contained. It never references the guide itself (no "According to Discover Canada..." framing inside the question).
- Every question has exactly 4 answer options, each a complete, plausible statement — not single words or numbers in isolation.
- Distractors are plausible-sounding but factually wrong statements.
- Exactly one option is correct.
- The official guide's sample section does not use true/false questions at all — only 4-option multiple choice. CitizenPass follows this: **new questions default to 4-option multiple-choice**. The `true-false` type in the data model is retained for schema flexibility but should only be used with a deliberate reason; all existing true/false questions have been converted to 4-option multiple-choice to match the official style.
